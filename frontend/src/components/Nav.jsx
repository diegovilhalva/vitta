  ...
  return (
    <>
      <nav className="h-[64px] flex items-center justify-between px-5 fixed top-0 z-[9999] bg-bg-default/80 backdrop-blur-md border-b border-gray-200 w-full shadow-sm">
        ...
        {userData.role === "user" && (
          <div className="hidden md:flex md:w-[50%] lg:w-[40%] h-[46px] bg-white shadow-sm rounded-full items-center gap-3 px-4 border border-gray-200">
            ...
            <span className="text-gray-500 truncate w-[25%]">{city}</span>
          ...
          {userData.role === "owner" ? (
            ...
            <>
              <div className="relative cursor-pointer">
                <FiShoppingCart size={22} className="text-primary" onClick={() => navigate("/cart")} />
                <span className="absolute -right-2 -top-2 text-xs font-bold text-white bg-primary rounded-full px-[6px] py-[1px]">{cartItems.length}</span>
              </div>
      ...
      {showSearch && userData.role === "user" && (
        <div className="fixed top-[64px] left-0 w-full bg-white shadow-md p-3 flex items-center gap-3 md:hidden z-[9998] animate-slideDown">

         
          <div className="flex items-center gap-1 px-3 py-2 bg-gray-100 rounded-full text-gray-600 text-sm flex-shrink-0">
            <FaLocationDot size={16} className="text-primary" />
            <span className="truncate max-w-[90px]">{city}</span>
          </div>

        
          <div className="flex items-center gap-2 flex-1 border border-gray-200 rounded-full px-3 py-2 shadow-sm">
            <IoIosSearch size={18} className="text-primary" />
            <input
              type="text"
              placeholder="Pesquisar restaurantes ou pratos..."
              className="flex-1 text-sm text-gray-700 outline-none"
